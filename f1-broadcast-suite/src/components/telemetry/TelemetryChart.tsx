...
interface Props {
  ...
  driverNumber: number;
  color: string;
  /** Callback so the parent page can get latest live values for G-Force / Tyre / Fuel */
...
export function TelemetryChart({ sessionKey, driverNumber, color, onFrame }: Props) {
  ...
  useEffect(() => {
    ...
    setFrames(next);
    // emit the latest frame to parent for G-force / tyre panels
    if (next.length && onFrame) onFrame(next[next.length - 1]);
  }, [data, onFrame]);
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Engine Audio — prominent banner */}
      <div
        className="flex items-center justify-between px-4 py-3 rounded-xl ring-1 ring-white/10 bg-[#0d0d0d]"
        style={{ borderColor: `${color}30` }}
      >
        <div className="flex items-center gap-3">
          <div className="size-2 rounded-full animate-pulse" style={{ backgroundColor: color }} />
          <span className="text-[10px] font-orbitron font-black uppercase tracking-widest text-[#888]">
            Engine Audio Synthesis
          </span>
          {latest && (
            <span className="text-[9px] font-jetbrains text-[#555]">
              RPM: <span style={{ color }}>{latest.rpm.toLocaleString()}</span> · Throttle: <span style={{ color: "#00FF66" }}>{latest.throttle}%</span>
            </span>
          )}
        </div>
        <EngineAudioPlayer
          ...
          <div key={c.key} className="bg-[#111] ring-1 ring-white/5 rounded-md p-4 h-44">
            <div className="flex justify-between mb-2">
              ...
              <span className="text-[10px] font-jetbrains" style={{ color: c.color }}>
                {latest ? Math.round((latest[c.key] as number) || 0) : "—"}
              </span>
            </div>
